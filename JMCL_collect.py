...
def GetInfo(issue, vol, driver, logger, start):
    ...
    for i in range(100):
        ...
        try:
            abstract = soup.select(f'#pb-page-content > div > main > div:nth-child(3) > div > div:nth-child({i}) > div.issue-item_metadata > div.issue-item_footer > div > div > span > p')[0].text
        ...
        if len(title) != 0:
            if title[0].text == 'Issue Editorial Masthead':
                break
        if title != []:
            res.append({'title': title[0].text, 'issue' : issue, 'vol' : vol, 'abstract' : abstract, 'doi' : title[0].get('id')})
